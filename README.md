...
| 判斷穩定度 | **仍是主要未解問題**。同一批評分點重跑 5 輪，主席開口次數在 1–5 次之間；三個判準變體（粗尺度、明確判準、兩段式）沒有一個能同時在兩場改善——把靈敏度調高，誤報就等比例升 |
| 「該講卻不講」 | 已解一半。根因之一是類型清單少一格：模型判定要介入、卻找不到類型可填而選「無」，被類型閘門滅掉（8/31 場 64% 的「要介入」是這樣沒的）。補上「發言權失衡」＋結構訊號後，標註的獨白窗口從 0/5 輪變成 5/5 輪；代價是慢路誤報中位 3 → 5。另一半（三軸打平被否決權擋掉）未解 |
| 雜務誤判 | 已修。調設備、找檔案曾被判為離題（5/5 輪），修正後 0/5，且未削弱對真正離題的偵測 |
| 介入類型覆蓋 | 七型中「假共識」「事實錯誤」從未在真實會議觸發 |
| STT 失效偵測 | 已實作，僅離線驗證 |
