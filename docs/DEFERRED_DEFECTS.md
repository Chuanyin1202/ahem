...
- **影響範圍**：「群體動力」抽屜目前是既有四塊（KPI／時間軸／主席的思考／發言
  分佈），沒有第五塊「會議節奏」視覺化。時間軸（`#timeline`）已經有類似資訊
  （發言段/主席開口點/忍住刻度/階段切換線），不是完全沒有節奏資訊，只是不是
  提案圖那種菱形圖示。
- **狀態**：未做，待補。下一棒如果要補，可以直接照抄提案圖那段 SVG 字串，把
  `160` 這個中心 x 座標依照 `state.meeting.phase`（發散期/呻吟區/收斂期）換算
  成三段式的指針位置。
